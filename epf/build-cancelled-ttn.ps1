...
if ($Modules.Count -eq 0) { throw "Form module not found under: $(Join-Path $Root 'EgaisCancelledTTN2026')" }
$Module = ($Modules | Where-Object { $_.FullName -match '\\Forms\\[^\\]+\\Ext\\Form\\Module\.bsl$' } | Select-Object -First 1).FullName
if ([string]::IsNullOrWhiteSpace($Module)) { $Module = $Modules[0].FullName }
...
if (!(Test-Path -LiteralPath $OutEpf)) { throw "EPF was not created. 1C exit code: $ExitCode" }
